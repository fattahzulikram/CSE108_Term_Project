...
class darkElf {
    ...
    static ArrayList<darkElf> DElist = new ArrayList<>();
    darkElf(double x, double y, enemyTOWER e, playerTOWER p, int side){
        mainstage = GameManager.gp;
        if(e!=null){
            skel = new ImageView(new Image("sample/Resources/DES2.png",50,40,true,false));
            act = new ImageView(new Image("sample/Resources/DES.png",55,45,true,false));
            enemy = e;
            ek = GameManager.StaticEnemy.getEnk();
        }
        else{
            skel = new ImageView(new Image("sample/Resources/DES3.png",50,40,true,false));
            act = new ImageView(new Image("sample/Resources/DES4.png",55,45,true,false));
            play = p;
    ...
    private void damageKING(){
        if(health>0 && enemy!=null && ek.getHealth()>0){
            ek.setHealth(ek.getHealth()-damage);
            this.health = this.health-ek.getDamage();
        }
        else if(health>0 && play!=null && pk.getHealth()>0){
            pk.setHealth(pk.getHealth()-damage);
            this.health = this.health - pk.getDamage();
        }
        else if((health>0 && enemy!=null && ek.getHealth()<0) || (health>0 && play!=null && pk.getHealth()<=0)){
            int some;
